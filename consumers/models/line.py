...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "org.chicago.cta.stations.table.v1": # stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("org.chicago.cta.station.arrivals."): # arrival topic
            self._handle_arrival(message)
        elif message.topic() == "TURNSTILE_SUMMARY":
            # SQL Turnstile Summary Topic
